...
def simulate(**pdata):
    ...
    mutation_rate = pdata['mutation_rate']  # 1e-3
    inital_state  = pdata['inital_state']   # 4
    r_rate        = pdata['r_rate']              # 2e-4
    ...
    fP            = pdata['fP']             # time-varying selection coefficient
    ...
    def mutation_step(pop):
        ...
        for genotype in genotypes:
            ...
            total_mu = seq_length * mutation_rate # for binary case
    ...
    f = open("%s/example/%s.dat"%(SIM_DIR,name), 'w')
...
def infer_const(**pdata):
    ...
    # obtain raw data
    data      = np.loadtxt("%s/sequence/%s.dat"%(SIM_DIR,name))
    q         = len(NUC)
    ne        = len(escape_group)
    escape_TF = [[0,0,0]]
    muMatrix  = [[0,mutation_rate],[mutation_rate,0]]
    ...
    flux_all   = get_flux(single_freq,escape_freq,muVec)
    ...
    np.savetxt("%s/%s/sc-%s-const.dat"%(SIM_DIR,dir,name),sc)
...
def infer_binary(**pdata):
        ...
        for t in range(len(nVec)):
            ...
            for n in range(len(escape_group)):
                ...
                for k in range(len(sVec[t])): # different sequences at time t
                    sVec_n = [sVec[t][k][i] for i in escape_group_n]
    ...
    def get_recombination_flux(p_wt,p_mut_k,trait_dis):
        flux = np.zeros((len(x),x_length))
    ...
    # calculate flux term (binary_case)
    def get_flux(x,ex,muVec):
        flux = np.zeros((len(x),x_length))
        for t in range(len(x)):
            for i in range(seq_length):
                aa = int(muVec[i])
                if aa != -1:
                    flux[t, aa] = mutation_rate * ( 1 - 2 * x[t, aa])
            for n in range(ne):
                for nn in escape_group[n]:
                    flux[t, x_length-ne+n] += mutation_rate * (1 - x[t, x_length-ne+n] - ex[t,n,nn] )
        return flux
    ...
    ####################### Inference (binary case) ############################
    # obtain raw data
    data      = np.loadtxt("%s/example/%s.dat"%(SIM_DIR,name))
    ne        = len(escape_group)
    ...
    p_wt,p_mut_k = get_p_k(sVec,nVec)
    ...
    flux_mu      = get_flux(single_freq,escape_freq,muVec)
    flux_rec     = get_recombination_flux(p_wt_freq,p_mut_k_freq,trait_dis)# recombination part
    ...
    def fun_advanced(a,b):
        ...
        p_wt_int, p_mut_k_int                             = interpolator_p(p_wt_freq,p_mut_k_freq,times,a)
        
        # use the interpolations from above to get the values of delta_x and the covariance matrix at the nodes
        flux_mu_int  = get_flux(single_freq_int, escape_freq_int,muVec)
        flux_rec_int = get_recombination_flux(p_wt_int,p_mut_k_int,trait_dis)# recombination part
    ...
    g = open('%s/example/c_%s_new.npz'%(SIM_DIR,name), mode='w+b')
...
def infer_multiple(**pdata):
    ...
    gamma_1s      = pdata['gamma_s']/totalT   #gamma_s/(totalT * 5)
    gamma_1p      = gamma_1s/10
    gamma_2c      = pdata['gamma_2c']       # 1000000
    gamma_2tv     = pdata['gamma_2tv']      # 500
    ...
    def fun_advanced(a,b):
        ...
        # flux_mu_int    = np.zeros((len(a),x_length))
        ...
        for t in range(len(a)): # right hand side of second half of the ODE system
            if len(etleft) <= t < len(etleft)+len(times):
                tt = t - len(etleft)
                result[x_length:2*x_length-ne,t] = (mat_prod_int[:x_length-ne,tt] + gamma_1s * b_1[:x_length-ne,t] + flux_mu_int[tt,:x_length-ne] - delta_x_int[tt,:x_length-ne]) / gamma_2c
                if ne != 0:
                    result[2*x_length-ne:,t]     = (mat_prod_int[x_length-ne:,tt] + gamma_1p * b_1[x_length-ne:,t] + flux_mu_int[tt,x_length-ne:] - delta_x_int[tt,x_length-ne:]) / gamma_2tv

                for i in range(len(p_sites)):#consider the individual site as time-varying site, use gamma and gamma_' for time varying part
                    for qq in range(q):
                        index = int (muVec[p_sites[i],qq]) # convert the index from all sequences to modified sequences
                        if index != -1:
                            result[x_length + index,t] = (mat_prod_int[index,tt] + gamma_1s * b_1[index,t] + flux_mu_int[tt,index] - delta_x_int[tt,index]) / gamma_2tv
    ...
    selection_coefficients = solution.sol(ExTimes)
    desired_coefficients   = selection_coefficients[:x_length,len(etleft):len(etleft)+len(times)] # removes the superfluous part of the array

    # save the solution with time-varying selection coefficient
    g = open('%s/%s/c_%s.npz'%(SIM_DIR,dir,name), mode='w+b')
