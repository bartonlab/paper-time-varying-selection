...
def main(args):
    ...
    if infer:
        ...
        ss_extend = data_sc['all']# time range:times
        
        try:
            solution = sp.integrate.solve_bvp(fun, bc, ExTimes, ss_extend, max_nodes=10000, tol=1e-4)
        except ValueError:
            print("BVP solver has to add new nodes")
            solution = sp.integrate.solve_bvp(fun_advanced, bc, ExTimes, ss_extend, max_nodes=10000, tol=1e-4)
        ...
        g = open('%s/output_try/c_%s_%d%s.npz'%(HIV_DIR,tag,time_step,name), mode='w+b')
