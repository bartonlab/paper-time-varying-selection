...
def main(args):
    ...
    for ti in range(len(time_all)):
        ...
        A_all[ti] = C_t + np.diag(gamma_1) # add gamma_1 to the diagonal
    ...
    def fun(time,s):
        ...
        for ti, t in enumerate(time): # loop over all time points, ti: index, t: time
            ...
            if t < 0 or t > sample_times[-1]:
                ...
                if t < 0:
                    gamma2_t = gamma2_all[0]
                else:
                    gamma2_t = gamma2_all[-1]
                ...
                gamma2_t = gamma2_all[time_index]
        
                # s'' = A(t)s(t) + b(t)
                dsdt[x_length:, ti] = (A_t @ s[:x_length, ti] + b_t) / gamma2_t
    ...
    g = open('%s/%s/new_sc_%s%s.npz'%(HIV_DIR, output_dir, tag, name), mode='w+b')
