...
def plot_his_sim(**pdata):
    ...
    box_lab = dict(left=0.05, right=0.15, bottom=0.10, top=0.45)
    box_tc  = dict(left=0.21, right=0.49, bottom=0.07, top=0.50)
    box_sc2 = dict(left=0.59, right=0.92, bottom=0.07, top=0.50)
    ...
    for i in range(len(tags)):
        ...
        ax_se.text(s_true_loc[i], 0.156, r'$s_{%s}$' % (tags[i]), color=colors[i], **tprops)
    ...
    for n in range(ne):
        ...
        mp.line(ax=ax_tc, x=[times], y=[tc_all[0][n]], colors=[C_group[n]], **pprops)
...
def plot_epitope(**pdata):
    ...
    # Plot only the selected epitopes
    if 'epi_plot' in pdata:
        epi_plot = pdata['epi_plot']
        temp_var_ec = []
        temp_traj_var = []
        temp_traj_group = []
        temp_var_tag = []
        ...
        temp_sc_all_ex = []
        for n in range(len(epitopes)):
            if var_tag[n] in epi_plot:
                temp_var_ec.append(var_ec[n])
                temp_traj_var.append(traj_var[n])
                temp_traj_group.append(traj_group[n])
                temp_var_tag.append(var_tag[n])
                temp_ytick.append(ytick[n])
                temp_yminortick.append(yminortick[n])
                temp_sc_sample.append(sc_sample[-(ne-n), :])
                temp_sc_sample_ex.append(sc_sample_ex[-(ne-n), :])
                temp_sc_all_ex.append(sc_all_ex[-(ne-n), :])
        var_ec = temp_var_ec
        traj_var = temp_traj_var
        traj_group = temp_traj_group
        var_tag = temp_var_tag
        ytick = temp_ytick
        yminortick = temp_yminortick
        sc_sample = np.array(temp_sc_sample)
        sc_sample_ex = np.array(temp_sc_sample_ex)
        sc_all_ex = np.array(temp_sc_all_ex)
        ne = len(var_ec)

    # PLOT FIGURE
    # set up figure grid
    fig = plt.figure(figsize=(SLIDE_WIDTH, SLIDE_WIDTH/(2*GOLDR)), dpi=500)
    ...
    pprops = { 'xticks':      xtick,
               ...
               'nudgey':      1.1,
               'plotprops':   {'lw': 2*SIZELINE, 'ls': '-', 'alpha': 0.5},
               'axoffset':    0.1,
               'theme':       'open',
               'combine'     : True}

    for n in range(ne):
        pprops['ylabel']          = var_tag[n] + ' escape variant\nfrequency'
        # add x axis and label at the bottom
        if n == ne - 1:
            pprops['xticklabels'] = xtick
            pprops['xlabel']      = 'Time (days)'
    ...
    plt.savefig('%s/CH%s%s.pdf' % (FIG_DIR,tag[-5:],output), facecolor = fig.get_facecolor(), edgecolor=None, **FIGPROPS)
