...
# load sequence data and get sVec, fVec, eVec
...
def main(args):
    ...
    def get_allele_frequency(sVec,fVec,eVec,muVec):
        T = len(fVec)
    ...
    def get_escape_fre_term(sVec, fVec):

        ex = np.zeros((len(fVec), seq_length, q), dtype=float)
    ...
    def get_p_k(sVec,fVec,escape_group,escape_TF):

        ne        = len(escape_group)
        n_k       = np.max([len(escape_group[n]) for n in range(ne)]) - 1

        p_mut_k   = np.zeros((len(fVec),ne, n_k, 3)) # 0: time, 1: all k point, 2: p_k, p_k-, p_k+
    ...
    seq_path = f"{HIV_DIR}/input/sequence/sigmoid/{tag}-seq-interp.dat"
    int_path = f"{HIV_DIR}/rawdata/interdata_{tag}.npz"
    ...
    times = np.loadtxt(seq_path, usecols=0)
    sample_times = np.unique(times).astype(int).tolist()
    time_all = np.linspace(raw_times[0], raw_times[-1], int(raw_times[-1]-raw_times[0]+1))

    # obtain sequence data and frequencies
    sVec,fVec,eVec = getSequence(data,escape_TF,escape_group)
    x_length,muVec = getMutantS(sVec, seq_length, q)
    x_length      += ne

    # get all frequencies, 
    # x: single allele frequency, xx: pair allele frequency
    x_all,xx_all         = get_allele_frequency(sVec,fVec,eVec,muVec)
    
    # ex: escape frequency
    if ne != 0:
        ex_all      = get_escape_fre_term(sVec,fVec)
        p_mut_k_all = get_p_k(sVec,fVec,escape_group,escape_TF)
    else:
        ex_all = 0
    ...
    flux_mu_all = cal_mut_flux(x_all, ex_all, muVec)
    dxdt, dxdt_node = cal_dx_all(x_all, sample_times)

    # extend the time range
    if raw_times[-1] > 100:
        interp_times = insert_time(raw_times)
    else:
        interp_times = time_all
    ExTimes  = get_ExTimes(interp_times)

    # Check if mutant epitope fixed and find the fixation time
    fixation_time = np.ones(ne) * (time_all[-1] + 100) # set a default fixation later than the last sample time
    for n in range(ne):
        x_epitope = x_all.T[-ne+n]
        ...
        if np.max(x_epitope[:-1]) == 1 and x_epitope[-1] == 1:
            ...
            for ti in range(len(x_epitope)-1):
                if x_epitope[ti] == 1 and x_epitope[ti+1] == 1:
                    fixation_time[n] = time_all[ti]
                    break
    
    # interpolate for recombination rates
    interp_r    = interp1d(raw_times, r_rates, kind='linear', bounds_error=False, fill_value=(r_rates[0], r_rates[-1]))
